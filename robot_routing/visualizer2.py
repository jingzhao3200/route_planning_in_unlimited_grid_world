...
if __name__ == '__main__':
    ...
    bounds = pc.bounds
    barriers = pc.barriers
    path = sc.path
    lasers = pc.rotating_lasers

    fig = plt.figure(frameon=False)
    x = np.arange(bounds[0]-5, bounds[2]+5, step=1)
    y = np.arange(bounds[1]-5, bounds[3]+5, step=1)

    for barrier in barriers:
        plt.scatter(barrier[0], barrier[1], c = 'black', s = 150, marker=(5, 0))
    for laser in lasers:
        plt.scatter(laser[0], laser[1], c='red', s=150, marker=(5, 2))
